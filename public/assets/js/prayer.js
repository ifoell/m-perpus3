function PrayTimes(method) {
    ...
    return {
        ...
        getTimes: function(date, coords, timezone, dst, format) {
            ...
            jDate = this.julian(date[0], date[1], date[2]) - lngi / (15 * 24);
        ...
        sunAngleTime: function(angle, time, direction) {
            ...
            var t =
                ...
                15 *
                DMath.arccos(
                    (-DMath.sin(angle) - DMath.sin(decl) * DMath.sin(lati)) /
                    (DMath.cos(decl) * DMath.cos(lati))
                );
        ...
        asrTime: function(factor, time) {
            var decl = this.sunPosition(jDate + time).declination;
            var angle = -DMath.arccot(factor + DMath.tan(Math.abs(lati - decl)));
        ...
        adjustTimes: function(times) {
            var params = setting;
            for (var i in times) times[i] += timeZone - lngi / 15;
...
function init() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(pos => {
            ...
            lngi = pos.coords.longitude;
            //console.log(lati, lng);
            pTime = prayTimes.getTimes(new Date(), [lati, lngi], tz / 100);
  ...
  `;
                ...
                .join("");
            document.querySelector(".pray-time").innerHTML += x;
        });
    } else {
